...
const Home = () => {
  ...
  useEffect(() => {
    const handleInView1 = async (entries) => {
      const [entry] = entries;
    ...
    const observer1 = new IntersectionObserver(handleInView1);
    if (textRef1.current) {
  ...
  useEffect(() => {
    const handleInView2 = async (entries) => {
      const [entry] = entries;
      if (entry.isIntersecting) {
    ...
    const observer2 = new IntersectionObserver(handleInView2);
    if (textRef2.current) {
  ...
  useEffect(() => {
    const handleInView3 = async (entries) => {
      const [entry] = entries;
      if (entry.isIntersecting) {
        await control3.start({
          opacity: 1,
          y: 0,
          transition: { duration: 0.5, ease: "easeInOut" },
        });
    ...
    const observer3 = new IntersectionObserver(handleInView3);
    if (textRef3.current) {
  ...
  useEffect(() => {
    const handleInView4 = async (entries4) => {
      const [entry4] = entries4;
      if (entry4.isIntersecting) {
        await control4.start({
          opacity: 1,
          y: 0,
          transition: { duration: 0.5, ease: "easeInOut" },
        });
    ...
    const observer4 = new IntersectionObserver(handleInView4);
    if (textRef4.current) {
      observer4.observe(textRef4.current);
    }
  
    return () => {
      if (textRef4.current) {
        observer4.unobserve(textRef4.current); // Disconnect the observer properly
      }
    };
  }, [control4]);
  ...
  { !showPage && (  <div
    ...
    >
      ...
      <motion.div
        ...
        transition={{ duration: 0.5, ease: "easeInOut" }}
        className="font-bold text-7xl italic"
      ...
      <motion.div
        ...
        className="font-bold italic text-5xl overscroll"
...
<motion.div
  className="absolute right-4 mt-10"
  initial={{ opacity: 0, scale: 0 }}
  animate={{ opacity: 1, scale: 1 }}
  transition={{ duration: 0.5, delay: 1 }}
>
<motion.button
  className="rounded-lg shadow-lg text-5xl italic font-bold border border-gray-800"
...
>
  Explore
      ...
      <div>
        ...
        >
          <motion.p className="font-bold italic text-7xl">
      ...
      <div>
        ...
        >
          <p>
            ...
            enlightenment, and discovery with our vast collection of books. Dive
            into gripping tales, explore new worlds, and enrich your mind with
            the wisdom of the ages. At Umair Library, we believe in the
      ...
      <div>
        ...
        >
          ...
          <ul className="text-md text-center mt-9 italic" style={{color:"gray"}}>

        <li >. Each book in our collection is carefully curated to ensure the highest standards of content and craftsmanship.</li>
        <li>. Browse, purchase, and enjoy your favorite titles from the comfort of your home, anytime, anywhere.</li>
